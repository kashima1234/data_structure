...
static int read_count_data(int *count_data)
{
    ...
    {
        puts("\nНекорректный ввод количества генерируемых данных!");
    ...
    {
        puts("\nНеверный ввод! "
                 "Количество генерируемых данных - число больше нуля!");
        return ERR_COUNT_DATA;
    }
...
{
    puts("\nВведите количество допустимых сравнений:");

    if (scanf("%d", count_cmp) != 1)
    {
        puts("\nНекорректный ввод количество допустимых сравнений!");
        return ERR_MENU_ITEM;
    }
...
static int read_elem_tree(int *num)
{
    ...
    if (scanf("%d", num) != 1)
    {
        puts("\nНекорректный ввод элемента дерева!");
        return ERR_MENU_ITEM;
    }
...
{
    puts("\nВведите элемент хеш-таблицы:");

    if (scanf("%d", num) != 1)
    {
        puts("\nНекорректный ввод элемента хеш-таблицы!");
        return ERR_MENU_ITEM;
    }
...
{
    puts("\nВведите элемент, который требуется найти:");
...
static int read_choise_hash_func(int *item)
{
    if (scanf("%d", item) != 1)
    {
        puts("\nНекорректный ввод номера хеш-функции!");
        return ERR_MENU_ITEM;
    }

    if (*item < 1 || *item > 2)
    {
        puts("\nНеверный ввод номера хеш-функции!");
        return ERR_MENU_ITEM;
    }
...
static int del_data(tree_t *tree, tree_t *balance_tree)
{
    ...
    if (rc == 0)
        ...
        puts("\nУдаляемого элемента в дереве нет");
...
static int find_hash_data(hash_table_t *table, hash_func_t hash)
{
    ...
    if (!is_empty_hash_table(table))
    {
        ...
        if (real_count_cmp > count_cmp)
        {
            ...
            puts("\nНеобходима реструктуризация хеш таблицы" );
            puts("\nВыберите пункт меню "
                      "смены хеш функции (номер 9)" );
        }
    }
    else
        puts("\nХеш-таблица пустая" );
